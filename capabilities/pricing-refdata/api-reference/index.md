---
layout: api-reference
title: Pricing & Reference Data API
permalink: /capabilities/pricing-refdata/api-reference/
service: pricing-refdata
---

# Pricing & Reference Data API Reference

Market pricing and reference data

<div class="api-explorer">
  <redoc spec-url="{{ site.baseurl }}/openapi/pricing-refdata.json"></redoc>
</div>

## Quick Links

- [OpenAPI Specification]({{ site.baseurl }}/openapi/pricing-refdata.json)
- [Use Cases Documentation]({{ site.baseurl }}/capabilities/pricing-refdata/)
- [Integration Guide]({{ site.baseurl }}/capabilities/pricing-refdata/pricing-refdata-api-documentation.html)

## Authentication

All API requests require authentication using JWT tokens. See the [Authentication Guide]({{ site.baseurl }}/capabilities/auth/api-documentation/) for details.

## Base URL

```
https://api.quub.exchange/v1/$\{service_slug\}
```

## Rate Limits

- **Standard**: 100 requests per minute
- **Premium**: 1000 requests per minute
- **Enterprise**: Custom limits available

## Support

For API support and questions:
- **Documentation**: [Use Cases]({{ site.baseurl }}/capabilities/pricing-refdata/)
- **Technical Support**: support@quub.exchange
- **Status Page**: https://status.quub.exchange
